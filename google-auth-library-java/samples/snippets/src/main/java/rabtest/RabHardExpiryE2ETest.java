...
/**
 * E2E test for Regional Access Boundary (RAB) Hard Expiry behavior.
 * Demonstrates that an expired RAB is not attached, but a background refresh is triggered.
 */
public class RabHardExpiryE2ETest {
  ...
  public static void main(String[] args) throws Exception {
    System.out.println("=== Starting Hard Expiry RAB Test ===");
    ...
    if (currentRab == null) {
      System.out.println("RAB lookup might have failed or is still pending. Cannot test hard expiry.");
      return;
    }
    ...
    System.out.println("\n--- Manipulating Refresh Time for Hard Expiry Test ---");
    // TTL is 6 hours (21600000 ms).
    // Hard expiry happens when currentTime > refreshTime + TTL
    // So we set refreshTime = currentTime - TTL - 10 seconds, 
    // making it fully expired.
    long sixHoursTenSecondsMs = (6 * 60 * 60 * 1000L) + (10 * 1000L);
    long newRefreshTime = System.currentTimeMillis() - sixHoursTenSecondsMs;
    ...
    System.out.println("Manually set RAB Refresh Time to: " + new Date(newRefreshTime).toString() + " (Past hard expiry)");

    System.out.println("\n--- Second Call to getRequestMetadata (Triggers Hard Expiry Refresh) ---");
    ...
    if (xAllowedLocations == null) {
      System.out.println("SUCCESS: x-allowed-locations is NOT attached because the RAB has hard-expired.");
    } else {
      System.out.println("FAILURE: x-allowed-locations should NOT be attached because it has hard-expired.");
    }
    ...
    System.out.println("\nRAB Refresh Time after hard-expiry refresh: " + new Date(refreshedRefreshTime).toString());
    System.out.println("RAB Value after hard-expiry refresh: " + refreshedEncodedLocations);

    // If the refresh succeeded, the new refreshTime should be very close to now,
    // which is significantly larger than the manipulated refreshTime (which was ~6 hours ago).
    if (refreshedRefreshTime > newRefreshTime + (5 * 60 * 60 * 1000L)) {
